...
/// Evaluate a segment condition, applying boolean operators (`or`, `and`,
/// `not`) over primary comparison/truthy terms. Precedence: `not` > `and` > `or`.
fn evaluate_condition(
    ...
    evaluate_or(condition.trim(), source, constants, accumulators)
}
        ...
        match in_quote {
            ...
            None => {
                ...
                if before_ws
                    && after_ws
                    && end <= bytes.len()
                    && expr[i..end].eq_ignore_ascii_case(op)
                {
...
fn evaluate_or(
    ...
    if terms.len() > 1 {
        return terms.iter().any(|t| evaluate_and(t, source, constants, accumulators));
    }
    evaluate_and(expr, source, constants, accumulators)
}
...
fn evaluate_and(
    ...
    let factors = split_top_level(expr, "and");
    if factors.len() > 1 {
        return factors.iter().all(|f| evaluate_not(f, source, constants, accumulators));
    }
    evaluate_not(expr, source, constants, accumulators)
}

/// `not` binds tightest; a leading `not` negates the term that follows.
fn evaluate_not(
    expr: &str,
    source: &DynValue,
    constants: &HashMap<String, DynValue>,
    ...
    if let Some(rest) = strip_leading_not(trimmed) {
        return !evaluate_not(rest, source, constants, accumulators);
    }
    evaluate_primary_condition(trimmed, source, constants, accumulators)
}

/// Strip a leading whole-word `not` (case-insensitive) followed by whitespace.
fn strip_leading_not(expr: &str) -> Option<&str> {
    if expr.len() >= 4 && expr[..3].eq_ignore_ascii_case("not") {
        let rest = &expr[3..];
...
/// Evaluate a primary term: a `path <op> value` comparison, or a truthy
/// check on the resolved path when no operator is present.
fn evaluate_primary_condition(
    condition: &str,
    source: &DynValue,
    constants: &HashMap<String, DynValue>,
    accumulators: &HashMap<String, DynValue>,
...
mod tests {
    ...
    fn bool_src(a: bool, b: bool) -> DynValue {
        DynValue::Object(vec![
            ("a".to_string(), DynValue::Bool(a)),
            ("b".to_string(), DynValue::Bool(b)),
        ])
    }

    #[test]
    fn test_evaluate_condition_and() {
        let no_c = HashMap::new();
        assert!(evaluate_condition("@.a and @.b", &bool_src(true, true), &no_c, &no_c));
        assert!(!evaluate_condition("@.a and @.b", &bool_src(true, false), &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_or() {
        let no_c = HashMap::new();
        assert!(evaluate_condition("@.a or @.b", &bool_src(false, true), &no_c, &no_c));
        assert!(!evaluate_condition("@.a or @.b", &bool_src(false, false), &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_not_truthy() {
        let no_c = HashMap::new();
        assert!(evaluate_condition("not @.a", &bool_src(false, false), &no_c, &no_c));
        assert!(!evaluate_condition("not @.a", &bool_src(true, false), &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_not_comparison() {
        let src = DynValue::Object(vec![("state".to_string(), DynValue::String("TX".to_string()))]);
        let no_c = HashMap::new();
        assert!(evaluate_condition("not @.state = \"CA\"", &src, &no_c, &no_c));
        assert!(!evaluate_condition("not @.state = \"TX\"", &src, &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_not_tighter_than_and() {
        // `not @.a and @.b` => (!a) and b
        let no_c = HashMap::new();
        assert!(evaluate_condition("not @.a and @.b", &bool_src(false, true), &no_c, &no_c));
        assert!(!evaluate_condition("not @.a and @.b", &bool_src(true, true), &no_c, &no_c));
        assert!(!evaluate_condition("not @.a and @.b", &bool_src(false, false), &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_and_tighter_than_or() {
        // `@.a or @.b and @.c` => a or (b and c)
        let src = |a: bool, b: bool, c: bool| {
            DynValue::Object(vec![
                ("a".to_string(), DynValue::Bool(a)),
                ("b".to_string(), DynValue::Bool(b)),
                ("c".to_string(), DynValue::Bool(c)),
            ])
        };
        let no_c = HashMap::new();
        assert!(evaluate_condition("@.a or @.b and @.c", &src(true, false, false), &no_c, &no_c));
        assert!(evaluate_condition("@.a or @.b and @.c", &src(false, true, true), &no_c, &no_c));
        assert!(!evaluate_condition("@.a or @.b and @.c", &src(false, true, false), &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_operator_word_in_quotes() {
        let src = DynValue::Object(vec![(
            "label".to_string(),
            DynValue::String("cats and dogs".to_string()),
        )]);
        let no_c = HashMap::new();
        // The `and` inside the quoted literal must not split the expression.
        assert!(evaluate_condition("@.label = \"cats and dogs\"", &src, &no_c, &no_c));
        assert!(!evaluate_condition("@.label = \"cats or dogs\"", &src, &no_c, &no_c));
    }

    #[test]
    fn test_evaluate_condition_case_insensitive_operators() {
        let no_c = HashMap::new();
        assert!(evaluate_condition("@.a AND @.b", &bool_src(true, true), &no_c, &no_c));
        assert!(evaluate_condition("@.a Or @.b", &bool_src(false, true), &no_c, &no_c));
        assert!(evaluate_condition("NOT @.a", &bool_src(false, false), &no_c, &no_c));
